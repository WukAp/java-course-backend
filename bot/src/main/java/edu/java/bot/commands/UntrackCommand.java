...
public class UntrackCommand implements Command {
    ...
    public SendMessage handle(Update update, Optional<UserStatus> status, TrackingDao trackingDao) {
        if (status.isPresent() && status.get().equals(UserStatus.WAITING_FOR_UNTRACKING_LINK)) {
    ...
    @Override
    public boolean supports(Update update, Optional<UserStatus> status) {
        return status.isPresent() && (status.get().equals(UserStatus.WAITING_FOR_COMMAND)
            ...
            || status.get().equals(UserStatus.WAITING_FOR_UNTRACKING_LINK));
    }
}
